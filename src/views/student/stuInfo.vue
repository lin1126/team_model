      ...
      <div class="stu-info-basic-card">
        <el-descriptions title="用户基本资料" direction="vertical" :column="3" border size="medium ">
          ...
          <el-descriptions-item label="年级、专业、班级">
            <span v-if="!isEdit">{{ formatClass }}</span>
            <el-input type="text" placeholder="请输入内容" v-model="formatClass" maxlength="20" show-word-limit v-if="isEdit" disabled> </el-input>
          </el-descriptions-item>
          <el-descriptions-item label="通信地址" :labelStyle="{ 'text-align': 'center' }" :contentStyle="{ 'text-align': 'center' }">
            ...
            <span>
              <el-input type="text" placeholder="请输入您的通信地址" v-model="stuInfo.place" maxlength="50" show-word-limit v-if="isEdit"> </el-input>
            </span>
          </el-descriptions-item>
        </el-descriptions>
      </div>
